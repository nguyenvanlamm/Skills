...
Note what this version cannot do: nothing compiles the project or greps the design system. The self-check is self-graded, so treat its score as a claim rather than a measurement. Use the skill itself when a real build gate is available.
...
Bundle the fonts in res/font/ OR fall back to FontFamily.Default — never reference an R.font that is absent.
...
HowToPlay: under 25 words + a visual. Layout survives a 320dp-wide screen.
...
Baseline: compileSdk/targetSdk 36 · minSdk 24 · JVM 17 · Kotlin 2.4.x · Compose BOM 2026.06.01 ·
...
- **Verify the claimed self-check.** Two greps catch most fabricated all-passes: `grep -rn '0x[0-9A-Fa-f]\{8\}' ui/screens` and `grep -rn 'coerceAtMost' game/`.
